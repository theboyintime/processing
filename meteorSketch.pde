...
// GLOBAL VARIABLES
PShape baseMap;
String csv[]; // always bring in data as a csv array
String myData[][];
PFont f;
...
void setup() {
  ...
  myData = new String[csv.length][6]; // data set has 6 columns
...
void draw() {
  ...
  for(int i=0; i<myData.length; i++){
    ...
    float markerSize = 0.05*sqrt(float(myData[i][2]))/PI; // meteor indicators
